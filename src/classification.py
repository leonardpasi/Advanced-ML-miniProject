"""
Investigating AdaBoost, + a little bit of Gaussian Process (10-fold cross validation
                                                            is performed here)

Note: all of these wont run without the data
...
#%% Gaussian Process Classifier 10-fold cross validation

kernel = 1.0*kernels.RBF(4.0) #  Amazing improvement wrt default kernel
model = GaussianProcessClassifier(kernel, n_restarts_optimizer=0)
kf = RepeatedStratifiedKFold(n_splits=10, n_repeats=1, random_state=1)
conf_matrices = []
train_times = []
test_times = []
accuracies_test = []
...
for train_index, test_index in kf.split(X_red, y):
   ...
   train_times.append(end_train - start_train) #[s]
   test_times.append(end_test - start_test) #[s]
...
# Load results
grid_result = joblib.load('./Grid_Search_Decision_Tree_10/grid_result_decision_tree_10.pkl')
grid = joblib.load('./Grid_Search_Decision_Tree_10/grid_decision_tree_10.pkl')
...
sns.heatmap(test_acc_means.reshape((n1,n2))*100, annot=annot, xticklabels=grid['n_estimators'],
            yticklabels=grid['learning_rate'], fmt='', ax=ax1, vmin=35, vmax=93, cbar=True, cmap="Blues")
...
sns.heatmap(train_acc_means.reshape((n1,n2))*100, annot=annot, xticklabels=grid['n_estimators'],
            yticklabels=grid['learning_rate'], fmt='', ax=ax2, vmin=35, vmax=93, cbar=True, cmap="Blues")
ax2.set_title('Train accuracy: mean $\pm$ standard deviation')
ax2.set_xlabel('Number of estimators')
ax2.set_ylabel('Learning rate')

#plt.savefig("Heat_Map_decision_tree_1.svg", bbox_inches='tight')
...
plt.savefig("ada_trees10_time_complexity_hyp.svg", bbox_inches='tight')
#%%


#%% Confusion Matrix for the selected model

base_estimator = DecisionTreeClassifier(max_depth=10, splitter="best")
model = AdaBoostClassifier(n_estimators = 200, learning_rate=1.0, base_estimator=base_estimator)
conf_matrices = []
...
sns.heatmap(mean_conf_matrix, annot=annot, fmt='', cbar=True, ax=ax, cmap="Blues",
            xticklabels=f_target, yticklabels=f_target)
        
ax.set_xlabel('Predicted class')
ax.set_ylabel('True class')
fig.suptitle('Confusion Matrix')

plt.savefig("Conf_Matrix_GPC.svg")                                
